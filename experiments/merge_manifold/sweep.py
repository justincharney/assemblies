"""Batch launcher for merge recording sweeps."""
...
MERGE_SCRIPT = REPO_ROOT / "experiments" / "merge_manifold" / "merge_recorder.py"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run merge sweeps")
    parser.add_argument(
        "--sweep-config", type=Path, required=True, help="JSON describing configs/seeds"
    )
    parser.add_argument(
        "--output-root", type=Path, default=Path("runs/merge_manifold"))
    ...
    tmp_dir = REPO_ROOT / "experiments" / "merge_manifold" / "configs" / "generated"
